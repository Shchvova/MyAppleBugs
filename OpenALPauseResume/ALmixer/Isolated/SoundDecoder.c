...
static SoundDecoder_Sample* s_listOfLoadedSamplesHead = NULL;
...
void SoundDecoder_FreeSample(SoundDecoder_Sample* sound_sample)
{
	 ...
	 */
	if(NULL != sample_internal->prev)
	{
		/* Connect the previous node to point to the next node to remove
		 * the current node.
		 */
		SoundDecoder_SampleInternal* previous_internal;
		previous_internal = (SoundDecoder_SampleInternal*)sample_internal->prev->opaque;
		/* Remember that next could be NULL which is the end of the list.
		*/
		previous_internal->next = sample_internal->next;
	}
	else
	{
		/* There is no previous member in the list, so the element
		 * we are going to remove is at the head.
		 * Make sure our head pointer shifts down one.
		 * Remember that next could be NULL which is the end of the list.
		 */
		s_listOfLoadedSamplesHead = sample_internal->next;
	}

	/* Now if there is actually a next node, 
	 * make sure its prev pointer is updated.
	 */
	if(NULL != sample_internal->next)
	{
		SoundDecoder_SampleInternal* next_internal;
		next_internal = (SoundDecoder_SampleInternal*)sample_internal->next->opaque;
		next_internal->prev = sample_internal->prev;
	}
...
)
{
	...
	/* This will insert at the head of the list. */
	internal_sample->next = s_listOfLoadedSamplesHead;
	if(NULL != s_listOfLoadedSamplesHead)
	{
		/* set the back pointer */
		SoundDecoder_SampleInternal* internal_next = (SoundDecoder_SampleInternal*)s_listOfLoadedSamplesHead->opaque;
		internal_next->prev = sound_sample;
	}
	/* reset the head to the new first element in the list */
	s_listOfLoadedSamplesHead = sound_sample;
